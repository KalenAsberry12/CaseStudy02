...
```
#Data Cleaning wanted to see the different factors of each data set rangign form education to WorkLife Balance. 
#I also wanted to group each predictive  variable with non predictors,  nominal quantitaive
```{r}
...
#The Department of the group I wanted to see the Number of employees per Department. The Department I was most instrested in was the Research & Development, Sales and Human Resource when Looking into the total number of the sample of 872. In Sales there are 273 that is a total 31.4%, Research and Development is 64.6% and Human Resource is 4.02%.
...
p
 ...
 #Looking at this histogram , Wanted to see the difference of the difference betwwen the Attriition of Age. I wanted to also see the Attrition of the yes and no of between each Age group  we stiil the highest points at 30. 
...
p <- plot_ly(alpha = 0.6) %>%
  ...
  layout(title = 'Histogram',
         barmode = "overlay",showlegend = F,xaxis = list(title = "Attrition(Orange) and No Attrition (Blue)"))
...
df_joblevel <- sqldf("select JobLevel,count(*) Attrition_count from data_cs02  where Attrition='Yes' group by JobLevel")
#rownames(df_joblevel) <- c('JobLevel1','JobLevel2','JobLevel3')
...
#
#knn_reg_target_ds <- data_cs02[,c(fin_num_predictors,fin_nom_qual_predictors,fin_ord_qual_predictors, "MonthlyIncome")] 
